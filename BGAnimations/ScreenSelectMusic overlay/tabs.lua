...
local tabNames = {"General", "MSD", "Scores", "Search", "Profile", "QuickOptions", "Goals", "Playlists", "Packs", "Tags"} -- this probably should be in tabmanager.
...
local sizeSingleNum = 14
local tabsizeIcons = {sizeSingleNum,sizeSingleNum}

local frameWidth = capWideScale(get43size(320), 230) / (#tabNames - 1)
...
local function tabs(index)
	local t = Def.ActorFrame {
		...
		SetCommand = function(self)
			...
			if getTabIndex() == index - 1 then
				...
				self:GetChild("TabSpriteShadow"):diffuse(getMainColor("positive"),0.3):diffusealpha(1)
				...
				self:GetChild("TabSpriteShadow"):diffuse(getMainColor("positive"),0.3):diffusealpha(0.3)
			end
		end,
	...
	t[#t + 1] = UIElements.QuadButton(1, 1) .. {
		Name = "TabBG",
		InitCommand = function(self)
			self:y(2):valign(0):zoomto(frameWidth, 30):diffusecolor(getMainColor("frames")):diffusealpha(0.7)
		end,
	...
	t[#t + 1] = LoadFont("Common Normal") .. {
		Name = "TabText",
		InitCommand = function(self)
			self:y(4):valign(0):zoom(0.3):diffuse(getMainColor("positive")):maxwidth(frameWidth * 3)
		end,
		BeginCommand = function(self)
			self:queuecommand("Set")
		end,
	...
	t[#t + 1] = Def.Sprite{
		Name = "TabSpriteShadow",
		Texture = THEME:GetPathG("" , "iconsTabs/" .. tabNames[index]),
		InitCommand = function(self)
			self:xy(-2,14):valign(0):zoomto(tabsizeIcons[1], tabsizeIcons[2])
		end
	}
	...
	t[#t + 1] = Def.Sprite{
		...
		Texture = THEME:GetPathG("" , "iconsTabs/" .. tabNames[index]),
		InitCommand = function(self)
			self:y(13):valign(0):zoomto(tabsizeIcons[1], tabsizeIcons[2])
		end
	}
	return t
end
